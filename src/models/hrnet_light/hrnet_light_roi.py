...
class HRNet(nn.Module):
    ...
    def __init__(self, num_keypoints=2):
        ...
        self.final_layer_heatmap = nn.Conv2d(32, num_keypoints, kernel_size=1)
        # Usa apenas branch1 (maior resolução) para gerar roi
        self.final_layer_roi = nn.Conv2d(32, 1, kernel_size=1)
    ...
    def forward(self, x):
        ...
        # ============ FINAL LAYER ============
        heatmaps = self.final_layer_heatmap(x1_final)  # (B, num_keypoints, 64, 64)
        roi = self.final_layer_roi(x1_final)  # (B, num_keypoints, 64, 64)
        ...
        roi = torch.nn.functional.interpolate(
            roi, size=(256, 256), mode="bilinear", align_corners=False
        )
