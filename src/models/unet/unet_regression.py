...
class UpSample(nn.Module):
    ...
    def forward(self, x1, x2):  # O "x2" é o valor recebido pela skip connection
       x1 = self.up(x1)
       x = torch.cat([x1, x2], 1)
       return self.conv(x)
...
class UNet(nn.Module):
    def __init__(self, in_channels=3, num_keypoints=2):
        super().__init__()
        ...
        # Head 2: Keypoints
        self.head_kp = nn.Conv2d(in_channels=64, out_channels=num_keypoints, kernel_size=1)
    ...
    def forward(self, x):
       ...
       roi_map = self.head_roi(up_4) # Colocar com sigmoide se usar o BCELoss 
