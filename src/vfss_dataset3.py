...
class VFSSImageDataset(Dataset):
    """
    Dataset PyTorch para frames de VFSS com suporte a augmentation via
    Albumentations.

    Modos de augmentation (parâmetro ``mode``):
    - ``'none'``    : Sem augmentation. Aplica apenas Resize(output_dim) e
                      converte para tensor.
    - ``'online'``  : A pipeline ``transform`` é aplicada a cada chamada de
                      ``__getitem__``, gerando amostras estocásticas diferentes
                      a cada epoch. Ideal para treino.
    - ``'offline'`` : A pipeline ``transform`` é aplicada **uma única vez** no
                      ``__init__`` (preload forçado). O cache armazena os
                      resultados transformados. Ideal para validação com
                      augmentation fixo ou para conjuntos pequenos onde se
                      quer reprodutibilidade total.

    A pipeline ``transform`` deve ser uma instância de
    ``albumentations.Compose`` construída com os ``additional_targets``
    corretos para os alvos selecionados (veja exemplos abaixo).

    Targets suportados (parâmetro ``target``, separados por ``'+'``):
    - ``'mask'``    : Máscara binária de segmentação (H×W, uint8).
    - ``'heatmap'`` : Mapa de calor gaussiano gerado a partir de keypoints.
    - ``'roi'``     : Região de interesse poligonal gerada a partir de
                      keypoints.
    - ``'points'``  : Coordenadas dos 4 cantos do bounding box rotacionado
                      (array NumPy shape (4, 2)).

    Exemplos de uso
    ---------------
    **Modo 'none' (apenas resize):**

    >>> ds = VFSSImageDataset(df, target='mask', mode='none')

    **Modo 'online' (augmentation a cada __getitem__):**
    ...
    ...         A.Resize(256, 256),
    ...     ],
    ...     additional_targets={'heatmap_c0': 'image', 'heatmap_c1': 'image', 'roi': 'mask'},
    ...     keypoint_params=A.KeypointParams(format='xy', remove_invisible=False),
    ... )
    >>> ds = VFSSImageDataset(df, target='mask+heatmap', mode='online',
    ...                       transform=transform)

    **Modo 'offline' (augmentation único, cacheado):**

    >>> ds = VFSSImageDataset(df, target='mask', mode='offline',
    ...                       transform=transform)

    Notas sobre ``additional_targets`` e ``keypoint_params``
    ---------------------------------------------------------
    - Se ``target`` inclui ``'heatmap'``, adicione
      ``additional_targets={'heatmap_c0': 'image', 'heatmap_c1': 'image'}``
      na pipeline. O heatmap tem 2 canais (C2 e C4) — ao declará-los como ``'image'``,
      o Albumentations aplica interpolação **bilinear** (igual à imagem) e
      preserva os valores float sem quantização. Internamente a classe replica
      o canal único para 3 canais antes de passar pela pipeline e extrai o
      primeiro canal na saída.
    - Se ``target`` inclui ``'roi'``, adicione
      ``additional_targets={'roi': 'mask'}`` (ou junto com heatmap).
      ROI é binário, portanto ``'mask'`` com ``NEAREST`` é correto.
    - Se ``target`` inclui ``'points'``, passe
      ``keypoint_params=A.KeypointParams(format='xy', remove_invisible=False)``
      e a classe injeta/extrai os keypoints automaticamente.
    - O ``Resize`` final **deve** estar dentro da pipeline Albumentations quando
      ``mode != 'none'``, pois o ``output_dim`` interno só é usado no modo
      ``'none'``.
    """
